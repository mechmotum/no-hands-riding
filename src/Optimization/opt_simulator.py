...
class Simulator:
    ...
    @inputs.setter
    def inputs(self, inputs: dict[Function, Callable[[float], float]]):
        ...
        for ri in inputs.values():
            if not callable(ri):
                raise TypeError(
                    f"Inputs should be of type {type(Callable[[float], float])} not "
                    f"{type(ri)}.")
    ...
    def initialize(self, check_parameters: bool = False) -> None:
        ...
        if check_parameters:
            ...
            print('state & inputs: ',state, inputs)
